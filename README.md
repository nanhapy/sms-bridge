...
青鸟传码将手机分享的验证码和文本发送到 Windows 接收端，接收端会显示通知并自动复制验证码。

> 名字取自李商隐“蓬山此去无多路，青鸟殷勤为探看”——青鸟是神话中的信使，正如它替你把验证码送到电脑。
...
2. 获取 `SMS Bridge Receiver_2.0.0_x64-setup.exe`，右键选择“以管理员身份运行”完成安装。安装过程需要提升权限，以配置 Windows 防火墙。
3. 安装后启动“青鸟传码”。接收端会在端口 8899 提供 TCP 接收和 UDP 发现服务。
